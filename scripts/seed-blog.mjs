/**
 * Seed 10 Grok-written suburb guide blog posts into the database.
...
const suburbs = [
  { name: "Bondi Beach", state: "NSW", city: "Sydney" },
  { name: "Toorak", state: "VIC", city: "Melbourne" },
  { name: "New Farm", state: "QLD", city: "Brisbane" },
  { name: "Cottesloe", state: "WA", city: "Perth" },
  { name: "Norwood", state: "SA", city: "Adelaide" },
  { name: "Manly", state: "NSW", city: "Sydney" },
  { name: "St Kilda", state: "VIC", city: "Melbourne" },
  { name: "Paddington", state: "QLD", city: "Brisbane" },
  { name: "Fremantle", state: "WA", city: "Perth" },
  { name: "Glenelg", state: "SA", city: "Adelaide" },
];
  ...
  const response = await fetch("https://api.x.ai/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: "You are an expert Australian real estate marketing consultant and content writer. You write detailed, accurate, and engaging suburb guides for real estate agents. Your content is original, SEO-friendly, and genuinely useful for agents.",
        },
...
async function main() {
  ...
  for (const suburb of suburbs) {
    ...
    const tags = JSON.stringify([suburb.name, suburb.state, suburb.city, "real estate", "suburb guide", "property market", "agent marketing"]);
  ...
  await conn.end();
  console.log("\n🎉 Blog seeding complete! 10 suburb guides published.");
}
